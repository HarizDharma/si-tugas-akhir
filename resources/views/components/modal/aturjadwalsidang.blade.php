...
<div id="aturJadwalSidang{{ $datamahasiswa['id'] }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="aturJadwalSidang{{ $datamahasiswa['id'] }}">
            ...
            <div class="px-6 py-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Atur Jadwal Sidang</h3>
                <div class="space-y-6 text-left">
                    {{--atur ulang jadwal sidang--}}
                    <div>
                        <form action="{{route('updateMahasiswaJadwalSidang', ['idMhs' => $datamahasiswa['mahasiswa_id']['id']])}}" method="POST">
                            @method('PUT')
                            @csrf
