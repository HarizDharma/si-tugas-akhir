<!-- Main modal -->
<div id="lihathasilSidangAkhir{{ $datamahasiswa['id'] }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="lihathasilSidangAkhir{{ $datamahasiswa['id'] }}">
            ...
            <div class="px-6 py-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Hasil Sidang Akhir Mahasiswa</h3>
...
{{--                        jika belum ada hasil sidang sempro maka outputnya berikut ini--}}
                    @elseif($datamahasiswa['mahasiswa_id']['hasil_sidang_akhir_id'] == 'Belum Ada Hasil Sidang Akhir')
                        <div>
                            <div>
                                Belum Ada Hasil Sidang Akhir
                            </div>
                        </div>
                    @endif
                </div>
