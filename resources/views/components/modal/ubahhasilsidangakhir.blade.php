<!-- Main modal -->
<div id="ubahHasilSidangAkhirMahasiswa{{ $datamahasiswa['id'] }}" tabindex="-1" aria-hidden="true" class="modal fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="ubahHasilSidangAkhirMahasiswa{{ $datamahasiswa['id'] }}">
            ...
            <div class="px-6 py-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Ubah Hasil Sidang Akhir Mahasiswa</h3>
                <form class="space-y-6" action="{{ route('ubahHasilSidangAkhirMahasiswa', ['id' => $datamahasiswa['mahasiswa_id']['id']]) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="dosen_penguji1" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Dosen penguji 1</label>
                        <input type="text" value="{{$datamahasiswa['mahasiswa_id']['hasil_sidang_akhir_id']['dosen_penguji1']}}" name="dosen_penguji1" id="dosen_penguji1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                    </div>
                    <div>
                        <label for="dosen_penguji2" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Dosen penguji 2</label>
                        <input type="text" value="{{$datamahasiswa['mahasiswa_id']['hasil_sidang_akhir_id']['dosen_penguji2']}}" name="dosen_penguji2" id="dosen_penguji2" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                    </div>
                    <div>
                        <label for="dosen_penguji3" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Dosen penguji 3</label>
                        <input type="text" value="{{$datamahasiswa['mahasiswa_id']['hasil_sidang_akhir_id']['dosen_penguji3']}}" name="dosen_penguji3" id="dosen_penguji3" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                    </div>
